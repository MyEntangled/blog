...
along its velocity direction. The diagram below depicts a simple 2D configuration 
with 5 velocities.
...
denoising operator as it filters out local non-equilibrium noise. The following 
figure is a simplified scheme in which the post-collision state $\sqrt{\mathbf{f}^{\mathrm{col}}}$
lies on the tangent line at a reference point on the manifold.

![Projection onto tangent space](/media/qlbm/projection.png "Projection")
...
replacing post-selected projection with a coherent evolution generated by $H = I
- D$. This is not yet a near-term recipe, but it points toward fully coherent
